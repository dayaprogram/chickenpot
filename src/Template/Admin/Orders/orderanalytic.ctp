...
<!--                    <a class="quick-btn" href="javascript:void(0)"> <i class="fa fa-shopping-cart" aria-hidden="true" ></i> <span> Sales </span> 
                        <span class="label label-success"><?php echo $transaction['sum'];?></span>  </a>-->
                    <a class="quick-btn" href="javascript:void(0)"> <i class="fa fa-first-order" aria-hidden="true"></i> <span> Orders </span> 
                        <span class="label label-success"><?php echo $transaction['total_order'];?></span> </a> 
                    ...
                    <div >
                          <form class="form-inline" id='ChartForm'>
                            ...
                            <div class="form-group">
                              ...
                              <input type="text" class="form-control" id="start_date" name="start_date" value="<?php echo date('01/m/Y'); ?>">
                            </div>
                            <div class="form-group">
...
<script>
    ...
    {
        $.post("<?php echo $this->request->webroot?>admin/orders/chart_ordergraph",$("#ChartForm").serialize(),function(data)
        {
        var range=[0,1];
        var table="";
        ...
        index++;
        table+="<tr><td>"+index+"</td><td>"+value.name+"</td><td>"+value.email+"</td><td>"+value.date+"</td><td>"+value.qty+"</td><td>"+value.amt+"</td><td><a href='<?php echo $this->request->webroot?>admin/orders/details/"+value.id+"'> <button class='btn-btn-info btn-xs'><i class='icon-eye-open'></i> View</button> </a></td></tr>";    
            
        });
      }
      else
      {
        table="<tr><td colspan=7>No Transaction Found</td></tr>";    
