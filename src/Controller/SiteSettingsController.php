...
//            $dt['gplus_url'] = $this->request->data['gplus_url'];           
    ...
    // side bar manegment function 28/08/2017(by surbhi)

    public function sidebar_manegment(){
          $this->loadModel('SiteSettings');
          $data = $this->site_setting();
    } 

    // end side bar manegment 
    ...
    public function sitedeliverycharges($id = null) {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            $dt['marchent_id']              = $this->request->data['marchent_id'];
            $dt['sharedcecret']             = $this->request->data['sharedcecret'];
            $dt['payment_mode']             = !empty($this->request->data['payment_mode'])?1:0;
            $sitesettings = TableRegistry::get('SiteSettings');
            $query = $sitesettings->query();
            if ( $query->update()->set($dt)->where(['id' => $data['id']])->execute() ) {
