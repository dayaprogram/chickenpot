    ...
    public function logo() {
        ...
        if ($this->request->is('post')) {
            ...
            if (!empty($this->request->data['site_logo']['name'])) {
             ...
             $arr_ext1 = array('jpg', 'jpeg', 'gif','png');
            if (!empty($this->request->data['site_favicon']['name'])) {
                $file1 = $this->request->data['site_favicon']; 
                 //print_r($file1);die;
                //put the data into a var for easy use
              //  $ext = substr(strtolower(strrchr($file['name'], '.')), 1);
                $ext1 = substr(strtolower(strrchr($file1['name'], '.')), 1);
              // print_r($ext1);die; //get the extension
                $fileName1 = 'favicon.' . $ext1;
               // print_r($fileName1);die; 
                if (in_array($ext1, $arr_ext1)) {
                    move_uploaded_file($file1['tmp_name'], WWW_ROOT . 'img' .DS. $fileName1);
                    if($data['site_favicon'] != ""){
                        if( $data['site_favicon'] != $fileName1 ){
                            $filePath1 = WWW_ROOT . 'img' .DS.$data['site_favicon'];
...
//            }
    ...
    public function sitedetail($id = null) {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            $dt['card_price'] = $this->request->data['card_price'];
